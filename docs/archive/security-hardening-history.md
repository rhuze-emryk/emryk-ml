# Archived security-hardening history

> This file is a non-authoritative historical record preserved verbatim below
> the archival notice. Current security guarantees and limitations are defined
> in [`SECURITY.md`](../../SECURITY.md); active risks are tracked in
...
- 2026-08-29 — boot-smoke publish gate restored (closes the 2026-08-11 P2 relaxation). The relaxation (`continue-on-error: true`, publish `needs: [build_scan]` only) existed because software-emulated boots flaked and blocked weekly releases; the root cause — guest checks racing systemd startup — was fixed 2026-08-21 by waiting for `multi-user.target` (`377662b`). The advisory legs then passed clean on consecutive publishing runs (2026-08-24 scheduled, 2026-08-29 dispatch; both variants each time), satisfying the row's closing test. Restored `needs: [build_scan, boot_smoke]` and dropped `continue-on-error`; deliberately KEPT the TCG software emulation (`accel=tcg`, hosted runners expose no `/dev/kvm`), the 120-minute budget, and `persist-credentials: false`. `tests/test-workflow-policy.sh` now asserts the gate and fails on any reappearance of `continue-on-error` in the boot_smoke block, so the gate cannot silently regress. Approval-prompt timing consequence: the Monday prompt now appears after boot evidence (~14:00 UTC), aligning with the 13:30 UTC cockpit digest.
- 2026-09-09 — `opencode.yml` removed, with its SECURITY-TODO P2 row, its SECURITY.md control row, and the five `tests/test-workflow-policy.sh` assertions that guarded it. The workflow had run once (2026-08-29, triggered by a cockpit comment; the OpenCode job itself was skipped), never had an `OPENAI_API_KEY` secret to run with, and carried a third-party action on an unversioned dev-branch pin with `id-token: write`. No use case existed alongside local Claude Code, so the P2 risk was retired by removal rather than by the pin ever moving to a tracked release.
