...
3. **CycloneDX SBOM** — complete package manifest generated by [syft](https://github.com/anchore/syft) from the published artifact, attached as a Sigstore-signed attestation. Verifiable with the same `gh attestation verify` command plus `--predicate-type https://cyclonedx.org/bom`. Generated by the nightly scan workflow (`.github/workflows/nightly-scan.yml`) rather than the per-push build, because stereoscope-based cataloging of this multi-GB bootc image cannot complete inside a free GHA runner's lost-comms watchdog window. The SBOM attestation therefore trails per-push provenance by up to 24 hours; consumers verifying provenance + signature get immediate trust, and SBOM-based supply-chain queries land within a day.
