...
3. **CycloneDX SBOM** — package manifest generated by [syft](https://github.com/anchore/syft) from the image's RPM database, attached as a Sigstore-signed attestation. Verifiable with the same `gh attestation verify` command plus `--predicate-type https://cyclonedx.org/bom`. Scope is RPM-installed packages only (every third-party payload in our build flow is installed via `dnf` — see `build_files/build.sh` and `private-ml-install.sh`); manually-installed binaries would not appear. If `build.sh` ever grows a non-RPM payload (e.g. `curl | tar -C /usr/local/bin`), this coverage assumption breaks and the SBOM/scan must move back to a full-image cataloger.
