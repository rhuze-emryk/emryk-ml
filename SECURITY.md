...
|---|---|---|
| Malicious image substitution (registry tampering, MITM, supply-chain compromise of GHCR) | All images from `ghcr.io/rhuze-emryk` are cosign-signed; installed hosts verify the signature on every pull | `/etc/containers/policy.json` + `/etc/pki/containers/rhuze-emryk.pub` shipped in the image |
...
The cosign signing key that gates trust for every pull from `ghcr.io/rhuze-emryk` is governed by a dedicated policy: see [KEY-POLICY.md](./KEY-POLICY.md). That document covers the threat model for the key itself, the rotation cadence (annual scheduled + on-incident), the rotation procedure, and the roadmap toward Sigstore keyless signing.
