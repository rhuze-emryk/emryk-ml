...
| CVEs in installed packages | Every CI build runs a Grype scan against the just-built image; results posted to the workflow job summary | `.github/workflows/build.yml` |
...
**Build-time package integrity.** Every third-party DNF repository vendored into the build (`build_files/*.repo`) enforces `gpgcheck=1`, so each RPM's own GPG signature — not just the repository metadata (`repo_gpgcheck=1`) — is verified at install time. The vendored `nvidia-container-toolkit.repo` sets `gpgcheck=1` even though upstream ships it with `gpgcheck=0`: vendoring lets us hold a stricter posture than upstream's compatibility-first default, and a package that is unsigned or signed with an untrusted key fails the build rather than landing silently.
